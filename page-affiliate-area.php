...
<main id="primary" class="site-main">
    ...
    <!-- W-9 Tax Fields: Injected via JavaScript into the registration form -->
    <!-- This ensures all tax collection code stays in the theme (GitHub backed up) -->
    <script>
    document.addEventListener('DOMContentLoaded', function() {
        // Find the AffiliateWP registration form
        var regForm = document.querySelector('.affwp-form.affwp-registration-form, form[action*="affiliate"], #affwp-register-form');

        if (!regForm) {
            // Try broader selectors
            regForm = document.querySelector('form[class*="affiliate"], form[class*="affwp"]');
        }

        if (regForm) {
            // Create W-9 fields container
            var w9Fields = document.createElement('div');
            w9Fields.id = 'microdos-w9-fields';
            w9Fields.innerHTML = `
                ...
                <p>
                    ...
                    <input type="text" name="affwp_w9_legal_name" id="affwp_w9_legal_name" required
                           style="width: 100%; background-color: #150f24; border: 1px solid #1f2b47; color: #e2e8f0; padding: 0.5rem; border-radius: 0.375rem;">
                </p>
            ...
            var submitButton = regForm.querySelector('input[type="submit"], button[type="submit"]');
            if (submitButton && submitButton.parentNode) {
                submitButton.parentNode.insertBefore(w9Fields, submitButton);
            } else {
                regForm.appendChild(w9Fields);
            }

            // Add form validation for W-9 fields
            regForm.addEventListener('submit', function(e) {
                ...
                for (var i = 0; i < requiredFields.length; i++) {
                    ...
                    if (!field || !field.value.trim()) {
                        e.preventDefault();
                        alert('Please fill in all required W-9 tax information fields.');
                        if (field) field.focus();
                        return false;
                    }
                }

                var cert = document.getElementById('affwp_w9_certification');
                if (!cert || !cert.checked) {
                    e.preventDefault();
                    alert('You must agree to the W-9 certification to register.');
                    if (cert) cert.focus();
                    return false;
                }
            });
